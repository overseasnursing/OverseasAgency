...
import type { CountryDetail, CountryRelatedGuide } from '@/types/countryDetail'

const CATEGORY_COLORS: Record<CountryRelatedGuide['category'], { bg: string; text: string; label: string }> = {
...
export function RelatedGuides({ country }: RelatedGuidesProps) {
  if (country.relatedGuides.length === 0) return null

  return (
    <section aria-labelledby="guides-heading">
      <h2 id="guides-heading" className="text-[22px] font-bold text-slate-800 mb-2">
        Related Guides
      </h2>
      <p className="text-[14px] text-slate-500 mb-6">
        In-depth guides for {country.name} nursing migration — exams, salaries, visa process, and comparisons.
      </p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {country.relatedGuides.map((guide) => {
