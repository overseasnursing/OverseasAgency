...
async function getBlogStats(): Promise<{ published: number; draft: number }> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export default async function AdminDashboardPage() {
  const [reviewStats, scamStats, totalAgencies, totalUsers, recentReviews, recentScams, blogStats] =
    await Promise.all([
      getReviewStats(),
      getScamReportStats(),
      getTotalAgencies(),
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="bg-white border border-slate-100 rounded-2xl p-5 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-[#FEF3C7] rounded-xl">
              <BookOpen size={16} className="text-[#92400E]" />
            </div>
            <span className="text-[15px] font-bold text-slate-800">Blog Posts</span>
          </div>
          <a href="/admin/blogs" className="text-[12px] text-primary font-semibold hover:underline flex items-center gap-1">
            Manage <ArrowRight size={11} />
          </a>
        </div>
        <div className="grid grid-cols-2 gap-3 max-w-xs">
          <div className="bg-[#DCFCE7] rounded-xl p-3 text-center">
            <p className="text-[20px] font-bold text-[#166534]">{blogStats.published}</p>
            <p className="text-[11px] text-slate-500">Published</p>
          </div>
          <div className="bg-[#F1F5F9] rounded-xl p-3 text-center">
            <p className="text-[20px] font-bold text-slate-600">{blogStats.draft}</p>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-5">
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { label: 'Write Blog Post',        href: '/admin/blogs/new',      color: 'bg-[#FFFBEB] text-[#92400E] hover:bg-[#FEF3C7]' },
