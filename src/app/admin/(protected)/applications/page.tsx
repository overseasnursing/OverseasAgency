...
import { getAllApplicationsWithJobs } from '@/lib/db/job-applications'
...
export default async function AdminApplicationsPage() {
  ...
  const applications = await getAllApplicationsWithJobs()

  return (
    <div className="flex flex-col gap-6">

      <div>
        <h1 className="text-[22px] font-bold text-slate-900 mb-1">All Applications</h1>
      ...
      {applications.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-2xl p-10 text-center">
          ...
          <p className="text-[13px] text-slate-400">
            Applications will appear here once nurses start applying to jobs.
          </p>
        </div>
      ) : (
              ...
              {applications.map((app) => (
                <tr key={app.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-5 py-3.5">
                    ...
                    <p className="text-[11.5px] text-slate-400 truncate max-w-[200px]">{app.email}</p>
                  </td>
