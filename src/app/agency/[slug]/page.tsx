...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${agency.name} Reviews, Fees & Scam Reports — OverseasNursing.com`
  const description = `Read verified nurse reviews of ${agency.name}. See agency fees (₹${agency.pricing.minCost / 100000}–${agency.pricing.maxCost / 100000}L), scam reports, visa success rate, and transparency score before you pay.`
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {agency.reviewCount > 0 ? (
                      ...
                      >
                        ...
                        {[1, 2, 3, 4, 5].map((i) => (
                          <Star key={i} size={12} fill={i <= Math.round(agency.googleRating!) ? '#F59E0B' : '#E2E8F0'} className={i <= Math.round(agency.googleRating!) ? 'text-[#F59E0B]' : 'text-slate-200'} />
                        ))}
                        <span className="font-semibold text-slate-800 ml-0.5">{agency.googleRating.toFixed(1)}</span>
                        <span className="text-[11.5px] text-slate-400">({agency.googleReviewCount})</span>
                      </a>
                    ) : (
                      <span className="text-slate-400 text-[13px]">No reviews yet</span>
                    )}
                  </div>
                  <div className="flex items-center justify-between">
